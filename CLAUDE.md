...
### [2026-03-25] S07 — FocusTrapDialog + React StrictMode = modal se inchide instant

**Cauza:** `focus-trap-react` (FocusTrap component) apeleaza `onDeactivate` in `componentWillUnmount`. In React StrictMode (dev mode), componentele sunt mount+unmount+remount. La primul unmount (StrictMode cycle), `onDeactivate` se apeleaza → `onClose()` → `setShowCreateWizard(false)` → modalul se inchide imediat dupa deschidere. Afecta TOATE modalele wrapped cu `FocusTrapDialog`.
**Fix:** Adaugat `mountedRef` guard in `FocusTrapDialog`. `onDeactivate` e wrapat cu `safeOnDeactivate` care verifica `mountedRef.current` setat cu `setTimeout(0)` — false in timpul StrictMode double-mount cycle, true dupa mount-ul real.
**Regula:** Cand folosesti `focus-trap-react` cu React StrictMode, NICIODATA nu pasa `onDeactivate` direct. Wrapeaza-l cu un guard ref care ignora deactivation in timpul StrictMode double-mount cycle. Pattern: `useRef(false)` + `setTimeout(() => ref.current = true, 0)` in useEffect.

### [2026-03-25] S07 — primary-blue (#2196F3) fail WCAG AA contrast pe alb si fundal deschis

**Cauza:** `primary-blue` era `#2196F3` (contrast ratio 3.12:1 pe alb, sub minim WCAG AA 4.5:1). Pe fundal `bg-blue-50` (#eff6ff) si `bg-white/95` (#f3f3f4) contrastul era si mai mic. Link-urile inline aveau doar `hover:underline` fara distinctie permanenta (WCAG link-in-text-block violation). `nuqs` v2 necesita `NuqsAdapter` care lipsea (cauza crash pe dashboard detail). Data-sources error state nu era afisata (hook returna `error` dar pagina nu-l folosea).
**Fix:** Schimbat `primary-blue` din `#2196F3` in `#1565C0` (contrast 5.82:1 pe alb, 5.35:1 pe blue-50). Actualizat in: tailwind.config.ts, chart-colors.ts, LineChart.tsx, WidgetConfigurator.tsx, opengraph-image.tsx, favicon.svg. Adaugat `underline` permanent pe link-uri inline (login terms/privacy, cookie banner, alerts upgrade, button link variant). Adaugat `NuqsAdapter` in app layout. Adaugat error state UI in data-sources page. Adaugat logs mock in E2E data-sources tests.
**Regula:** NICIODATA culori cu contrast sub 4.5:1 pe ORICE fundal prezent in app (nu doar alb pur). Verifica si pe `bg-blue-50`, `bg-white/95`, `bg-gray-50`. Link-uri inline TREBUIE sa aiba `underline` permanent (nu doar `hover:underline`). Verifica WCAG cu axe-core.
